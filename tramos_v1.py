...
when reading the files related to tramos, their sheets' names are not uniform
'''
import pandas as pd
...
class Productivity_Analizer:
    def __init__(self, sheet_name: str, directory: str, cm: str = None, file_prefix: str = 'rdat_metro'):
        ...
        for f in os.listdir(directory):
            if f.startswith(file_prefix):
                df_tmp = pd.read_excel(directory + f)   # sheet names are not uniform
                columns.append(df_tmp.columns)
                df = df.append(df_tmp)

        # filter if required
        if cm is not None:
            df = df[df['SIGLASCM_P'] == cm]
            if df.shape[0] < 1:
    ...
    def hourly_reports(self):
        ...
        for i, col in enumerate(cols):
            ...
            time_df = pd.DataFrame(hour_data[col].dt.hour)
    ...
    def daily_reports(self):
        # BUILD ALL DATES
        start_date = self.df['TRA_F_REALTERM'].min().date()
        end_date = self.df['TRA_F_REALTERM'].max().date()
        ...
        # instances_by_day = self.df['TRA_F_REALTERM'].dt.floor('d').value_counts()
        instances_by_day = self.df['TRA_F_REALTERM'].astype(str)[:4].value_counts()
        print(instances_by_day)
    ...
    def monthly_report(self):
        instances_by_month = self.df['F_LIQ_ENLACE'].groupby([self.df.F_LIQ_ENLACE.dt.year, self.df.F_LIQ_ENLACE.dt.month]).agg('count')
        ...
        d['cols'] = ('Mes', 'Reportes')
        d['xys'] = dict(zip(instances_by_month['year_month'], instances_by_month['F_LIQ_ENLACE']))
        self.plotting_data.append(d)
